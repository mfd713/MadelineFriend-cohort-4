...
namespace SustainableForaging.UI
{
    public class View
    {
        ...
        public void DisplayForagers(List<Forager> foragers)
        {
            ...
            {
                io.PrintLine($"{index++}: {forager.FirstName} {forager.LastName}, {forager.State}");
            }
        }

        public void DisplayKgItemReport(Dictionary<Item, decimal> query)
        {
            foreach (var item in query)
            {
                io.Print($"{item.Key.Name}: ");
                io.PrintLine($"{item.Value:F} kg");
            }
        }

        public void DisplayValCategoryReport(Dictionary<Category,decimal> query)
        {
            foreach (var category in query)
            {
                io.Print($"{category.Key}: ");
                io.PrintLine($"{category.Value:C}");
            }
        }
    }
}
